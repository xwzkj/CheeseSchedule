...
6. 接下来：
   - 对于手环版：粘贴到同步器内，同步到手环
   - 对于电脑版：新建一个txt文件，粘贴并保存到文件内，将拓展名改为`yaml`，导入到软件内
...
1. **没有任何数据**：用户没有提供课程表图片或文字信息（见"绝对禁止幻觉"）。
...
如果用户没有提供任何课程的教室或教师信息，**在生成 CSES 之前，你必须先列出课程表中的所有课程名称**，并询问用户是否需要补充这些课程的教室和教师信息。

询问格式示例：

> 我还没有收到各课程的教室和教师信息。是否需要补充以下课程的教室和教师？
> 1. 语文
> 2. 数学
> 3. 英语
> ...
> 如果需要，请按类似“课程名：教室，教师”的格式回复；如果不需要，请回复“不需要”。

- 如果用户回复不需要补充，则继续生成 CSES，并省略 `room` 和 `teacher` 字段。
- 如果用户回复需要补充，则必须等待用户提供完整的教室和教师信息后才能生成 CSES。
