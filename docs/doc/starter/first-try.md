...
您可以直接使用该编辑器创建一个全新的课程表数据。

### 编辑时间表

在编辑课程数据前，您需要先进入`时间表编辑`页面，创建每节课程对应的时间槽位，用于后续设置课程。
...
奶酪课程表支持多周轮换课表，如果您的学校采用大小周等制度，可以前往`设置`页面，对多周轮换进行配置。
...
您可以进入`设置`页面，对CSES文件进行导入导出。

如果您没有CSES文件，可以尝试[使用AI生成CSES数据](../other/ai-cses.md)。
...
如果您之前使用过该软件，拥有可用的配置文件`config.json`，则适用于这一方法。

进入`设置`页面，执行`打开配置文件所在位置`，将定位到的`config.json`文件替换为您的配置文件，然后重启软件即可。
...
您可以在`临时换课`页面对今天的课程进行临时调整，不会影响您的课程表数据！

![override-and-main-window](../assets/starter/first-try/override-and-main-window.jpg)

在这张图片中，您可以看见右侧的主窗口已经显示了今天的课程表。

主窗口分为两个区域：

1. 上方的小组件区域：显示自定义的小组件
2. 下方的课程表区域：显示今天的课程表
   - 现在是这节课课前：卡片展开、背景较浅
   - 现在是这节课课上：卡片展开、背景较深
   - 现在不是这节课：卡片收起
